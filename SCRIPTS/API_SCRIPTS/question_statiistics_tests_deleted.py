...
class OnlineAssessment:
    ...
    def calculate_qn_stats(self, excel_datas, hirepro_token, at_token):
        automation_test_ids = []
        hirepro_test_ids = []
        for i in excel_datas:
            if i.get('tenant') == 'automation':
                ...
                hirepro_test_ids.append(int(i.get('testId')))
        automation_test_ids = set(automation_test_ids)
        automation_test_ids = list(automation_test_ids)
        hirepro_test_ids = set(hirepro_test_ids)
        hirepro_test_ids = list(hirepro_test_ids)
        print(hirepro_test_ids)

        if automation_test_ids:
            calculate_question_stats_context_id = CrpoCommon.calculate_question_statistics_for_tests(at_token,
                                                                                                     automation_test_ids)
            self.task_status = CrpoCommon.job_status(at_token, calculate_question_stats_context_id)
            self.current_task_status = self.task_status['data']['JobState']
            while self.current_task_status != 'SUCCESS':
                self.task_status = CrpoCommon.job_status(at_token, calculate_question_stats_context_id)
                current_task_status = self.task_status['data']['JobState']
        if hirepro_test_ids:
            calculate_question_stats_context_id = CrpoCommon.calculate_question_statistics_for_tests(hirepro_token,
                                                                                                     hirepro_test_ids)
            self.task_status = CrpoCommon.job_status(hirepro_token, calculate_question_stats_context_id)
            self.current_task_status = self.task_status['data']['JobState']
            while self.current_task_status != 'SUCCESS':
                self.task_status = CrpoCommon.job_status(hirepro_token, calculate_question_stats_context_id)
                current_task_status = self.task_status['data']['JobState']
                ...
                for excel_question in excel_datas:
                    if act_question.get('questionId') == int(excel_question.get('questionId')):
                        ...
                        write_excel_object.compare_results_and_write_vertically(excel_question.get('testCases'),
                                                                                None, self.row, 0)
                        write_excel_object.compare_results_and_write_vertically(excel_question.get('questionId'),
                                                                                None, self.row, 2)
                        ...
                        write_excel_object.compare_results_and_write_vertically(int(excel_question.get('correct')),
                                                                                act_correct, self.row, 11)
                        write_excel_object.compare_results_and_write_vertically(
                            excel_question.get('partialCorrect'), act_partial_correct, self.row, 13)
                        write_excel_object.compare_results_and_write_vertically(excel_question.get('facilityValue'),

...
class CodingEval(unittest.TestCase):
    def test_Coding_Eval(self):
        try:
            ...
            mycursor.execute(
                'delete from test_results where testuser_id in (select id from test_users where test_id = 6082 and login_time is not null);')
            conn.commit()
            print 'Test result deleted'
            mycursor.execute(
                'delete from candidate_scores where testuser_id in (select id from test_users where test_id = 6082 and login_time is not null);')
            conn.commit()
            print 'Candidate score deleted'
            mycursor.execute(
                'delete from test_user_login_infos where testuser_id in (select id from test_users where test_id = 6082 and login_time is not null);')
            conn.commit()
            print 'Test user login info deleted'
            mycursor.execute(
                'update test_users set login_time = NULL, log_out_time = NULL, status = 0, client_system_info = NULL, time_spent = NULL, is_password_disabled = 0,config = NULL, client_system_info = NULL, total_score = NULL, percentage = NULL where test_id = 6082 and login_time is not null;')
            conn.commit()
        ...
        # Read from Excel
        # --------------------------------------------------------------------------------------------------------------
        wb = xlrd.open_workbook("/home/muttumurgan/Desktop/AutomationScripts/PythonWorkingScripts_InputData/"
                                "Assessment/Coding_Eval.xls")
        ...
        while n < sh1.nrows:
            ...
            data0 = {"LoginName": "admin", "Password": "SLWS-124", "TenantAlias": "automation",
                     "UserName": "admin"}
            ...
            wb_result.save("/home/muttumurgan/Desktop/AutomationScripts/PythonWorkingScripts_Output/Assessment/"
                           "Coding_Evaluation_Check.xls")
            n += 1
            row_num += 1

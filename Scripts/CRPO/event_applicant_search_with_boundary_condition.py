...
class Excel_Data:
    ...
    # ------------------------------------------------------------------------------------------------------------------
        wb = xlrd.open_workbook('/home/muthumurugan/Desktop/Automation/PythonWorkingScripts_InputData/'
                                'CRPO/Event/Event_applicant_Search_Boundary_Condition.xls')
    ...
    def all(self):
        ...
        for i in range(0,tot_len):
            ...
            if self.total_api_count  == self.Query_Result1:
                ...
                self.ws.write(self.rownum, 4, mismatched_id, self.__style2)
                print self.query
            self.wb_result.save('/home/muthumurugan/Desktop/Automation/PythonWorkingScripts_Output/CRPO/EventSearch/'
                + self.__current_DateTime + '_Event_Applicant_SearchExpected.xls')
            # print statusCode, " -- ", b

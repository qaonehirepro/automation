...
class ChainingOfTwoTests:
    ...
    def __init__(self):
        ...
        # self.ws.write(1, 28, "2nd Login Expected URL for T2", self.black_color_bold)
        # self.ws.write(1, 29, "2nd Login Acual URL for T2", self.black_color_bold)
    ...
    def final_report(self, row_value, next_test_info, excel_data, next_tu_infos, second_login_nexttest_infos):
        ...
        #                   second_login_nexttest_infos.get('domain'))
        # self.ws.write(row_value, 28, excel_data.get('expectedDomainUrlForSecondTest'), self.black_color)
        # self.ws.write(row_value, 29, second_login_nexttest_infos.get('domain'), self.color)
...
for data in excel_data:
    ...
    if submit_token:
        ...
        # Below condition is to fetch score for non Hirepro and Non VET tests Currently Talentlens is not in use
            ...
            assessment_common_obj.initiate_vendor_score(crpo_headers, data.get('candidateId'),
        ...
        second_login_stat = assessment_common_obj.login_to_test(login_name=data.get('loginName'),
                                                                password=data.get('password'),
                                                                tenant='automation',
                                                                type_of_test=data.get('firstTestIsTypeOf'))
        ...
        if second_login_stat.get('login_response').get('status') == 'KO':
            if second_login_stat.get('login_response').get('error').get('isAutoTest'):
                ...
                if nextTestFlags.get('isShortlisted') == False:
                    ...
                    t2_login_second_time = assessment_common_obj.login_to_test(
                        ...
                        tenant='automation',
                        type_of_test=second_login_stat.get('firstTestIsTypeOf'))
